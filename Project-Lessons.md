* **Business Environment**
* **Start the Project**
* **Plan the Project**
* **Lead the Team**
* **Support the Project Team**
* **Close the Project/Phase**

---

## 🔹 1. Business Environment

**Focus:** Aligning the project with organizational strategy, compliance, and value delivery.

**Lessons / Key Points:**

* Understand **why** the project exists (business case, benefits management plan).
* Projects must align with **strategic objectives** and **value delivery**.
* Monitor compliance with **laws, regulations, and policies** (ex: labor, environmental, pharma regulations).
* Adapt to **external influences** (market, political, environmental, economic).
* Be aware of **organizational change management**—projects exist in a changing environment.

---

## 🔹 2. Start the Project

**Focus:** Defining scope at a high level and authorizing the project.

**Lessons / Key Points:**

* Use the **project charter** to formally authorize the project.
* Identify and analyze **stakeholders early** → create a stakeholder engagement strategy.
* Ensure **project objectives** are clear, measurable, and realistic.
* Clarify **roles & responsibilities** from the start.
* Establish initial **governance structure** (decision-making, escalation paths).

---

## 🔹 3. Plan the Project

**Focus:** Building a roadmap for scope, schedule, budget, risks, and resources.

**Lessons / Key Points:**

* Plan collaboratively with the team and stakeholders.
* Develop the **project management plan** (scope, schedule, cost, quality, resource, risk, procurement, stakeholder, communication).
* Identify and prioritize **requirements** → create the WBS.
* **Risk planning is essential**: maintain a risk register and strategies.
* Balance **predictive planning** with **Agile adaptability** where needed.
* Ensure alignment with **baselines** (scope, cost, schedule).

---

## 🔹 4. Lead the Team

**Focus:** Inspiring, motivating, and directing the project team.

**Lessons / Key Points:**

* Effective leadership = **servant leadership + situational leadership**.
* Build a **high-performing team** → trust, empowerment, collaboration.
* Foster **team culture** and shared ownership of goals.
* Address **conflicts quickly** (best PMP answer: collaborate/problem-solve).
* Use **emotional intelligence** and adaptability in leadership.
* Encourage **continuous improvement** and learning.

---

## 🔹 5. Support the Project Team

**Focus:** Enabling the team with resources, removing blockers, and maintaining engagement.

**Lessons / Key Points:**

* Provide the team with **resources, training, and tools**.
* Empower decision-making within agreed boundaries.
* Remove organizational and process **impediments**.
* Support **knowledge transfer** and open communication.
* Encourage **stakeholder collaboration** and transparency.
* Track **team performance** and provide feedback regularly.

---

## 🔹 6. Close the Project / Phase

**Focus:** Formally ending the project or phase and capturing lessons.

**Lessons / Key Points:**

* Verify that **deliverables meet acceptance criteria**.
* Obtain **formal sign-off** from the customer/sponsor.
* Transition deliverables to **operations or next phase**.
* Release project resources (team, contracts, vendors).
* Document **lessons learned** → update **organizational process assets (OPA)**.
* Celebrate team achievements and provide recognition.

---

* Business Environment
    - Foundation
        - Project
        - The Evolution of Project Management
        - Project Management Life Cycles and Development Approaches
            - Predictive 
            - Adaptive
            - Hybrid
        - Project Management Office (PMO)
            - Supportive PMO
            - Controlling PMO
            - Directive PMO
            - Agile Center of Excellence (ACoEs) / Value Delivery Office (VDO)
        - Organizational Project Management (OPM)
            - Project Management
            - Program Management
            - Portfolio Management
            - Operations Management
        - Organizational Structures
            - Functional
            - Project Oriented
            - Matrix
            - Composite
        - Project Management Principles / 12 Principles
        - Performance Domains / 8 Performance Domains
        - The Agile Manifesto for Software Development
            - Four Values
            - 12 Principles
    - Strategic Alignment
        - PMI Talent Triangle
            - Ways of Working
            - Power Skills
            - Business Acumen
        - Strategic Alignment and Business Management Skills
        - Strategic Management Elements and Frameworks
        - Organizational Influences 
            - Enterprise Environmental Factors (EEFs)
                - Internal 
                - External
            - Organizational Process Assets (OPAs)
                - Process, Policies and Procedures
                - Corporate Knowledge Base
        - Get to know the External Busisness Environment
    - Project Benefits and Value
        - Business Value
        - Examine Business Value
        - Types of Business Value
            - Financial Gain
            - New Customers
            - Social Benefit
            - First to Market
            - Improvement
            - Regularization
        - Needs Assessment / Obtain Data for the Project
        - Business Documents
            - Business Case
            - Benefits Management Plan
            - Benefit Measurement Methods
                - Cost-benefit Analysis
                - Internal Rate Of Return (IRR)
                - Return on Investment (ROI)
        - Project Selection Using Present Value (PV) and Net Present Value (NPV)
        - How OKRs Help Deliver Business Value
        - Incremental Value Delivery
    - Organizational Culture and Change Management
        - Change Management
        - Manage Organizational Change Impacts on Projects
        - Get to Know Organizational Cultures and Styles
        - Risk, Culture and Change in Organizations
        - Change Management Framework
        - Actions to Support Change
        - Plan for Change
        - Organizational Transformation for Project Practitioners
    - Project Governance
        - Project Governance Components
        - Governance in Adaptive Projects
        - Governance Board / Project Board / Steering Committee
        - Governance Defines Escalation Procedures
        - Governance and Life Cycles
        - Governance Checkpoints : Phase Gates and Iterations
        - Project Phases Relationships
        - Apply Governance to Predictive Project Phases
    - Project Compliance
        - Compliance 
        - Compliance Requirements
        - Compliance Categories Classification
        - Compliance Threats
        - Treat Compliance as a Project Objective
        - Compliance : Five Best Practices
            1. Documentation 
            2. Risk Planning 
            3. Compliance Council
            4. Compliance Audit
            5. Compliance Stewardship
* Start the Project
    - Identify and Engage Stakeholders
        - Typical Project Stakeholders
        - Stakeholder and Communication Management
        - Stakeholder Identification
            1. Who are they?
            2. What's their relationship to the project?
        - Assess Stakeholders
        - Create Stakeholder Register
        - Know your Stakeholders : Go Beyond Job Titles
        - Stakeholder Mapping
        - Directions of Influence
        - Salience Model
        - Stakeholder Perceptions
        - Capture Stakeholder Feedback and Perceptions
        - Plan to Communcate with Stakeholders
        - Communication Requirements Analysis
        - Communication Methods and Technologies
            1. Push Communication 
            2. Pull Communication
            3. Interactive Communication
        - Communication Challenges / Considerations
        - Communication Model
            1. Cross-Cultural Communication Model
        - Stakeholder Engagement Strategy
        - Stakeholder Engagement Assessment Matrix (SEAM)
    - Form the Team
        - Create a Collaborative Team Culture
        - Project Team Formation
            1. Self-Organizing Team
            2. Servant Leadership
        - Tuckman's Ladder of Team Development
        - Project Manager Role in Adaptive Teams
        - Leadership and Management Models
            1. Centralized
            2. Distributed
        - Hybrid Team Formation
        - Project Team Composition
        - Project Team Roles
        - Identify Project Resource Requirements
        - T-Shaped People and Self-Organizing Teams
        - Standards for Creating a Balanced, Fair and Trusting Team
        - Experts and Expert Judgement
        - Focus on Team Strengths
        - Team Norms
        - PMI Code of Ethics and Professional Conduct
        - Team Charter and Ground Rules
        - Team Communication
        - Colocated, Virtual or Both
        - Virtual Team Challenges
        - Running Virtual Teams
        - Virtual Team Communication Technology
        - Address Virtual Team Member Needs
    - Build Shared Understanding
        - Seek Consensus for the Project Among the Team and Stakeholders
        - Building a Shared Understanding
        - Project Visison Statement
        - Holistic Understanding of the Project
        - How to create a Holistic Understanding of the Project
        - Refers to Business Case and Business Needs
        - Negotiate and Agree on Project Success Criteria
        - Help Everyone Understand the Vision
        - Project Charter
        - Kickoff Meeting
    - Project Approach
        - Predictive Life Cycle
        - Adaptive Life Cycle
        - Hybrid Life Cycle
        - Assess Complexity
            - The Stacey Complexity Model
        - Suitability Filter
        - Scrum
        - Scrum Ceremonies
            1. Sprint Planning
            2. Daily Scrum
            3. Sprint Review - Can include Demo
            4. Sprint Retrospective
        - Agile Ceremonies
            1. Product Strategy Meeting
            2. Daily Standup or Standup
            3. Backlog Refinement
            4. Project Restrospective 
* Plan the Project
    - Planning Projects
        - Planning Starts with a Project Management Plan
            1. Subsidiary Plans
            2. Baselines
            3. Additional Components
        - Project Documents
        - Collaborative Planning : Adaptive and Hybrid Development Approaches
    - Scope
        - Adaptability and Resilience in Planning
        - Rolling Wave Planning
        - Minimum Viable Product (MVP) or Minimum Business Increment (MBI) : Planning for Work Incrementally 
        - Product Roadmap
        - Milestones
        - Scope Planning
        - Requirements
        - Document Requirements
        - Requirements Traceability Matrix
        - Requirements Management Plan
            1. Plan
            2. Track
            3. Report on Requirements Activities
        - Types of Requirements
            1. Project
            2. Product
            3. Quality
            4. Business
            5. Stakeholder
            6. Transition / Readiness
        - Collect Requirements Process
        - Data Gathering
        - Requirements Prioritization
            1. MoSCoW Analysis : Developed by Dai Clegg
            2. Kano Model : Developed by Noriaki Kano
            3. Paired Comparison Analysis : Developed by LL Thurston
            4. 100 Points Method : Developed by Dean Leffingwell and Don Widrig
        - Represent Data
            1. Mind Mapping
            2. Affinity Diagram
        - Context Diagrams
        - Prototyping e.g Storyboarding
        - Scope Management Plan
        - Project Scope Statement
        - Product Analysis Methods
        - Create the Work Breakdown Structure (WBS)
        - Decompose Work in the WBS
        - WBS Dictionary
        - Scope Baseline
        - Release and Iteration Planning
        - Backlogs, Prioritize and Refine the Backlog
        - User Stories, Story Maps, Roadmaps
        - Epics, Features 
    - Schedule
        - Schedule Management Plan
        - Schedule Management Plan Components
        - Break Down Projects Activities
        - Activity Dependency Types
            1. Mandatory
            2. Discretionary
            3. External
            4. Internal
        - Precedence Relationships
        - Types of Precedence Relationships
            1. Finish to Start
            2. Finsh to Finish
            3. Start to Start
            4. Start to Finish
        - Lags and Leads in Precedence Relationships
        - Activity Duration Estimate Terminology
        - Estimating Techniques
            1. Analogous / Top Down
            2. Parametric
            3. Three-Point
            4. Bottom-Up
        - Three-Point Estimation
            1. Tringular Distribution (Average)
            2. BETA Distribution (PERT Average)
        - Critical Path Method
        - Network Diagram with Date and Dependencies
            1. Critical Path
            2. Forward Pass
            3. Backward Pass
            4. Float
        - The Project Schedule
        - Schedule Presentation Formats
            1. Roadmap
            2. Gantt Chart
            3. Milestone Chart
            4. Project Schedule Network Diagram
        - Resource Optimization
            1. Smoothing
            2. Levelling
        - Schedule Compression Techniques
            1. Fast Tracking
            2. Crashing
        - Adaptive Scheduling Approaches
            1. On-Demand (Kanban/Lean-based)
            2. Time-boxed / Iterative
        - Agile Release Planning : Story Mapping
        - Measure Effort, Not Time
            - Relative sizing 
            - Story Points
            - Planning Poker
        - Definition of Ready (DoR)
        - Definition of Done (DoD)
        - Reprioritize Sprint / Iteration Backlog
        - Hybrid Scheduling Models
    - Resources
        - People and Equipment
        - Resource Management Plan
        - Assign Resources and Allocate Responsibilities
        - Use Resource Calendars
        - Responsibility Assignment Tools
            - Responsibility Assignment Matrix (RAM) or RACI
                1. RESPONSIBLE
                2. ACCOUNTABLE
                3. CONSULT
                4. INFORM
        - Filling Resource Needs : Make or Buy? or Borrow
        - Plan the Procurement Strategy
        - Procurement Management Plan
        - Procurement Documents 
        - Bid and Proposal Activities
        - Source Selection Criteria
        - Qualified Vendors
        - Contracts
        - Negotiate Productive Relationships
        - Contrcat Types
            1. Cost Plus Fixed Fee
            2. Cost Plus Incentive Fee
            3. Cost Plus Award Fee
            4. Firm Fixed Price
            5. Fixed Price Incentive Fee
            6. Fixed Price with Economic Price Adjustments
            7. Pre-approved Vendors or International Payments
            8. Time and Materials Contracts
        - Components of Contracts
    - Budget
        - Budget Planning
        - Predictive Budget Planning
        - Resource Cost
        - Estimate Costs
        - Contingency Reserves
        - Budget Considerations
    - Risks
        - Conditions of Uncertainty
        - Project Risks
        - Business Context
        - Create Risk Strategy
        - Define / Refine Risk Management Approach
        - Inherent Risk
        - Risk Identification Techniques
            1. Risk Breakdown Structure (RBS)
            2. Delphi Technique
            3. Affinity Diagram
        - Create Risk Probability and Impact Definitions
        - Probability and Impact Matrix
        - Risk Register
        - Risk List
        - Quantitative Risk Analysis Methods
        - Qualitative Risk Analysis Methods
        - Risk Response Strategies
    - Quality
        - Cost of Quality
        - Quality Management Plan
        - Compliance Requirements
        - Quality Standards and Regulations
        - Quality Metrics, Checklists and Processes
        - Quality Methods for Continuous Improvement
            - Six Sigma / Lean Six Sigma
            - Kaizen
            - Plan - Do - Check - Act (PDCA)
            - Agile Methods - Scrum, Kanban, Crystal Methods
    - Integrate Plans
        - Integrating Plans
        - Change Control
        - Plan for Complexity and Change
        - How to Approach Complex Plans
            - Fail Fast and Self-Correct!
* Lead the Project Team
    - Craft Your Leadership Skills
        - Power Skills
        - Guidelines for Developing Inclusive Leadership Competencies
        - Leadership Skills & Competencies
        - Interpersonal and Team Skills
        - Leadership Styles
        - Servant Leadership
        - Adopt a Growth Mindset
        - Team Building
        - Tuckman Stages of Team Development
        - Balance Team Tone with Sense of Urgency
        - Virtual Team Member Engagement
        - Virtual Team Best Practices
    - Create a Collaborative Project Team Environment
        - Where and How the Team Works
        - Agile Space for Hybrid Teams
        - Work Information Management Systems
        - Importance Of Artifacts
        - Information Storage and Distribution Good Practices
        - Standardize Artifacts
        - Tailor Artifacts
        - Maintain Artifacts
        - Version Control
    - Empower The Team
        - Empower Teams with EI and Fluid Communication
        - Empowerment, Unity, Autonomy
        - Crafting a Balanced, Fair and Trusting Team
        - Create Psychological Safety and Embrace Diversity
        - Motivational Theories / Approaches
            1. Maslow's Hierarchy of Needs
                - Slef-Actualization
                - Esteem
                - Belonging / Social
                - Safety
                - Physiological
            2. Herzberg's Motivation-Hygiene Theory or Two-Factor Theory
                - Hygiene Factors
                    1. Salary
                    2. Work Environment
                - Motivators
                    1. Advancement
                    2. Recognition
                    3. Engagement
            3. McGregor's Theory X and Y
                - Therory X is Authoritarian
                - Theory Y is Participative
            4. McClelland's Achievement Motivation Theory 
                - Achievement
                - Affiliation
                - Power
        - Uphold Team Charter and Ground Rules
        - Use Rewards and Recognition
        - Decision Making 
            1. Empower the Team to Act
            2. Opportunities to Empower the Team
        - Decision Making Methods
            1. Voting (Consensus-driven, based on data)
                - Unanimity e.g Delphi Technique
                - Majority
                - Plurality
            2. Multicriteria Decision Analysis (Data-driven)
            3. Autocratic Decision Making (Leadership-driven, based on data) 
        - Voting Methods
            1. Fist of Five
            2. Planning Poker
            3. Dot Voting
            4. Roman Voting (Thumbs)
            5. Polling
        - Display Task Accountability
    - Support Team Member Performance
        - Manage and Lead
        - Assess Team Member Performance
        - Performance Assessment Tasks
        - Personality Indicators : Look Beyond Introvert / Extrovert
        - Personality Measurement Tools
            1. Big Five Personality Model (OCEAN)
            2. Myers-Briggs Type Indicator (MBTI)
            3. DISC
        - Use Personality Research to Coach Team Members
        - Emotional Intelligence
            1. Self Awareness
            2. Self Regulation
            3. Motivation
            4. Empathy
            5. Social Skills
        - Motivation Elements
            1. Achivement / Drive
            2. Commitment
            3. Initiative
            4. Optimism
    - Communication and Collaborate with Stakeholders
        - Monitor Stakeholders and Their Engagement
        - Communication Management Plan
        - Communication : Two Ways
            1. Active Listening
            2. Effective Feedback
        - Reports and Formal Communication
        - How to Collaborate
        - Use infromation Radiators : Keep infromation Visible
            1. Kanban Boards
            2. White Boards
            3. Wikis
            4. Fishbowl Windows
        - Collaboration Activities
        - Communicate and Collaborate to Negotiate
        - Meetings
        - Stakeholder Engagement Assessment Matrix (SEAM)
    - Training, Coaching and Mentoring
        - Foster a Knowledge-Sharing Culture
        - Training
        - Coaching
        - Mentoring
        - How to Acquire Required Competencies
        - Plan for Training, Coaching and Mentoring
        - Know the Value of Training, Coaching and Mentoring
        - Elements of Training
        - Coach Teams and Individuals in Project Management
        - Coach Groups and Individuals
        - Self-Organizing Teams Collaborate and Learn
        - Measure Training Outcomes
        - Maintain Mentorships
    - Manage Conflict
        - Why conflict Management Matters
        - Causes of Conflict : Context
        - Conflict as Part of Team Culture
        - How to Handle Conflict
        - Use Leas'Levels of Conflict
            1. A Problem to Solve
            2. Disagreement
            3. Contest
            4. Fight / Flight
            5. Intractable Situation
        - Use Interpersonal Skills to Manage Conflict
            1. Emotional Intelligence
            2. Influencing
            3. Leadership
            4. Decision-Making
            5. Active Listening
        - Conflict Management Approaches
            1. Smooth / Accommodate
            2. Withdraw / Avoid
            3. Compromise / Reconcile
            4. Force / Direct
            5. Collaborate / Problem Solve
* Support the Project Team Performance
    - Implement Ongoing Improvements
        - Continuous Improvement (CI)
        - Kaizen
        - Assess Current CI Methods
            1. Lean Six Sigma
            2. Kaizen
            3. Crystal Methods
            4. Capability Maturity Model Integration (CMMI)
        - Conduct Restrospectives : Review and Improve Methods
        - Improve Your Improvement Methods
        - Pareto Chart / 80-20 Rule
        - Update Processes and Standards
        - Lead with an Improvement MindSet
    - Support Performance
        - Project Team Leadership Objectives
        - Manage with Objectives, Tolerances, Thresholds
        - The Project Manager's Role
            1. Centralized Model
        - Team Roles and Responsibiliries to Support Performance
            1. Project Manager
            2. Scrum Master / Agile Coach
            3. Product Owner
        - Optimize Communication
        - Team Charter
        - Use Feedback to Support High Performance
        - Support Team Task Accountability
        - Show Roles and Responsibilities (RAM / RACI)
            1. Responsibility Assigment Matrix (RAM)
            2. Responsibile, Accountable, Consult, Infrom (RACI)
        - Curate Knowledge as an Asset
        - Incorpoate Knowledge Transfer Opportunities
        - Knowledge Management : Three Levels
            1. Individual
            2. Project
            3. Organization
        - Learn the Right Way to Motivate Your Team
        - Continuously Realign Team Efforts with Value Delivery
        - Check on Artifacts Maintenance
    - Evaluate Project Progress
        - Guidelines to Measuring Performance : Only Measure What Matters
            1. Scope
            2. Schedule
            3. Budget
            4. Resources
            5. Quality
            6. Risk
        - Report on Performance : Tailor If Required
            1. Milestone Schedule
            2. Quality Reports
            3. Earned Value Management (EVM) Reports
            4. Variance Analysis Reports
            5. Work Performance Reports
            6. Dashboards
        - Monitor Scope
        - Scope Validation
        - Measure Schedule Performance : Methods
        - Schedule Management Tools
        - Visualize Performance
        - Burn Charts 
            1. Burndown (Iteration)
            2. Burnup (Release)
        - Task Board
        - Estimate Velocity
            - Velocity Chart
        - Continuous Flow Approaches
            - Measure Throughput, Lead and Cycle Time
                1. Work In Progress (WIP)
                2. Lead Time
                3. Cycle Time
                4. Throughput
            - The Cumulative Flow Diagram
        - Budget Challenges
        - Earned Value Management (EVM)
        - EVM Measures for Schedule Control
        - EVM Measures for Cost Control
        - EAC / ETC Analysis
        - Manage and Lead Resources
        - Physical Resource Management
        - Update Resource Allocation
        - Handle Changes and Contract Disputes
        - Quality Management Guidelines
        - Evaluate and Manage Quality
        - Quality Audit
        - Control Quality Tools
        - Control Quality Process
        - Data Visualization
            1. Quality Tool - Cause and Effect Diagram
            2. Scatter Diagram
            3. Control Chart
            4. Histogram and Pareto Chart
        - Ensure Quality of Processes and Product
        - Verify Deliverables
        - Evalute and Manage Risk
        - Monitor Risk
        - Review Your Reserves
        - Risk Register
        - Manage Compliance as the Highest Priority
        - Examine Business Value
    - Manage Project Issues and Impediments
        - Problem Vocabulary
            1. Impediments
            2. Obstacles
            3. Blockers
        - Issue or Impediment? Just Solve the Problem!
        - Risk and Issues
        - Issue Resolution
        - Discover and Solve Impediments Using Scrum
        - Remove Impediments
    - Manage Project Changes
        - Causes of Project Changes
        - Be a Changemaker and a Change Leader
        - Monitor the External Business Environment
            1. PESTLE : Political, Economic, Social, Technical, Legal, Environmental
            2. TECOP : Technical, Economic, Operational, Political
            3. VUCA : Volatility, Uncertainty, Complexity, Ambiguity
        - Manage Change
        - Change Management Process Flowchart
        - Change Requests 
            1. Corrective Action
            2. Preventive Action
            3. Defect Repair
            4. A Change
        - Change Control Systems : Change Control Board
        - Manage Contract Changes and Resolve Problems
        - Contract Change Control System
        - Types of Contract Changes
            1. Administrative Changes
            2. Contract Modification
            3. Supplemental Agreement
            4. Constructive Changes
            5. Termination Of Contract
        - Legal Concepts When Managing Disputes
        - Process, Adjudicate and Communicate Claims
        - Update Project Management Plan
* Close the Project/Phase
    - Project / Phase Closure
        - Why Projects or Phases Close 
            1. Fulfillment
            2. Premature or Forced Closure
        - Close Project or Phase Activities
        - Transitions 
            1. Handovers
            2. Handover Readiness
            3. Handover Activities
        - Paying and Closing Contracts
        - Finalizing Contracts
    - Benefits Realization
        - Early and Long Term Benefits Realization
        - Benefits Transition and Sustainment
        - Benefits Management Plan
        - Benefits Owner
        - Verify Benefits Realization
    - Knowledge Transfer
        - Knowledge Management During Closing
        - Conduct Project Retrospective
        - Finalize Lessons Learned
        - Consolidating Lessons Learned
        - Final Report : Summary of Project / Phase performance result